...
class Admin_data extends CI_Model{
    ...
    function search_data_pengguna($id_user){
   	...
   	*/
   	...
   	function get_data_menu($limit, $offset){
   		$this->db->select('kd_menu, nama, harga, status,stok');
   		$this->db->from('menu');
      $this->db->offset($offset);
      $this->db->limit($limit);
   		$query=$this->db->get();
    ...
    /*method get data menu*/
    function get_data_menu1($limit, $offset){
      $this->db->select('kd_menu, nama, harga, status,stok');
      $this->db->from('menu');
    ...
    /*
    #method kasus data set menu
    */
    /*method get data paket menu*/
    function get_data_setmenu($limit, $offset){
      $this->db->select('set_menu.kd_menu as kd_menu, set_menu.kd_set as kd_set, set_menu.kd_jenis as kd_jenis, set_menu.kd_paket as kd_paket, set_menu.update_time as update_time, set_menu.create_time as create_time, menu.nama as nama, menu.keterangan as keterangan, menu.harga as harga, menu.status as status, menu.stok as stok, menu.*, jenis_set.nama_jenis as nama_jenis ,jenis_set.*, nama_set.nama_set as nama_set, nama_set.*, paket_menu.nama_paket as nama_paket, paket_menu.*');
      //$this->db->select('kd_set , nama_set, update_time, create_time');
      $this->db->from('set_menu');
      $this->db->JOIN('menu','menu.kd_menu=set_menu.kd_menu');
      $this->db->JOIN('jenis_set','jenis_set.kd_jenis=set_menu.kd_jenis');
      $this->db->JOIN('nama_set','nama_set.kd_set=set_menu.kd_set');
      $this->db->JOIN('paket_menu','paket_menu.kd_paket=set_menu.kd_paket');
      $this->db->offset($offset);
      $this->db->limit($limit);
      $query=$this->db->get();
      return $query->result();
	...
	/*
  /*method get data nama set*/
    function get_data_namaset($limit, $offset){
     
      $this->db->select('kd_set , nama_set, update_time, create_time');
      $this->db->from('nama_set');
  ...
  /*
  /*method get data nama set*/
    function get_data_jenisset($limit, $offset){
     
      $this->db->select('kd_jenis , nama_jenis, update_time, create_date');
      $this->db->from('jenis_set');
  ...
  /*
  /*method get data nama set*/
    function get_data_paketmenu($limit, $offset){
     
      $this->db->select('kd_paket , nama_paket, date_mulai, date_akhir, waktu_mulai, waktu_akhir, update_time, create_time');
      $this->db->from('paket_menu');
  ...
  /*
    ...
    /*method tambah stok*/
    function tambah_adjustment($data){
      $this->db->insert('adjustment',$data);
      //$this->db->insert('stok_barang',$data);
      $retVal = ($this->db->affected_rows()!=0) ? true : false ;
      return $retVal;
    }
    /*method tambah history*/
    ...
    /*method update data stok*/
    function jum_stok_before($data){
      $this->db->select('stok');
      $this->db->from('menu');
      $this->db->where('kd_menu',$data);
      $query=$this->db->get();
      $data2= $query->result_array();
      return ($data2[0]['stok']);
      //return $this->db->update('menu', $data);
    }
    function update_jum_stok($data=array()){
      $this->db->where('kd_menu', $data['kd_menu']);
      //$a= $data['jum_stok_before']+$data['jum_stok'];
      $this->db->update('menu', $data);
      $retVal = ($this->db->affected_rows()!=0) ? true : false ;
      return $retVal;
      //return $this->db->update('menu', $data);
   	...
   	/*method hitung jumlah stok*/
   	function getcount_stok(){
  		$this->db->select('kd_stok');
  		$this->db->from('stok_barang');
  		return $this->db->get()->num_rows();
	  }
    /*method hitung jumlah stok*/
    function getcount_adjustment(){
      $this->db->select('kd_adjustment');
      $this->db->from('adjustment');
      return $this->db->get()->num_rows();
    }

   	/*method lihat detail stok*/
   	function get_detail_stok($kd_stok){
   		$this->db->select('*');
   		$this->db->from('stok_barang');
   		$this->db->where('kd_stok',$kd_menu);
   		$query=$this->db->get();
   		return $query->result();
   	}

   	/*method get data stok*/
   	function get_data_stok($limit, $offset){
   		$this->db->select('stok_barang.kd_stok, stok_barang.kd_menu, menu.nama, stok_barang.lokasi_asal, stok_barang.lokasi_tujuan, stok_barang.pengirim, stok_barang.penerima, stok_barang.jum_stok');
   		$this->db->from('stok_barang');
      $this->db->JOIN('menu','menu.kd_menu=stok_barang.kd_menu');
      $this->db->offset($offset);
      $this->db->limit($limit);
   		$query=$this->db->get();
   		return $query->result();
    ...
    function get_search_stok($nama_menu){
      $this->db->select('stok_barang.kd_stok as kd_stok, stok_barang.kd_menu as kd_menu, stok_barang.lokasi_asal as lokasi_asal, stok_barang.lokasi_tujuan as lokasi_tujuan, stok_barang.pengirim as pengirim, stok_barang.penerima as penerima, stok_barang.jum_stok as jum_stok, menu.* ');
      //$this->db->select('stok_barang.*,menu.*');
      $this->db->from('stok_barang');
      $this->db->JOIN('menu','menu.kd_menu=stok_barang.kd_menu');
      $this->db->like('stok_barang.pengirim',$nama_menu);
      $this->db->like('stok_barang.penerima',$nama_menu);
      $this->db->like('menu.nama',$nama_menu);
      $this->db->like('menu.keterangan',$nama_menu);
      $query=$this->db->get();
      return $query->result();
    }

   	/*method update data stok*/
   	function update_data_stok($data=array(),$kd_menu){
   		$this->db->where('kd_stok',$kd_menu);
      ...
      //return $this->db->update('menu', $data);
   	...
   	/*method hapus stok*/
   	function delete_data_stok($kd_menu){
   		$this->db->where('kd_stok',$kd_menu);
      ...
      /*method get data adjustment*/
    function get_data_adjustment($limit, $offset){
      $this->db->select('adjustment.kd_adjustment, adjustment.kd_menu, menu.nama, adjustment.note_menu, adjustment.jum_stok');
      $this->db->from('adjustment');
      $this->db->JOIN('menu','menu.kd_menu=adjustment.kd_menu');
      $this->db->offset($offset);
      $this->db->limit($limit);
      $query=$this->db->get();
      return $query->result();
    ...
    function get_search_adjustment($nama_menu){
      $this->db->select('adjustment.kd_adjustment as kd_adjustment, adjustment.kd_menu as kd_menu, adjustment.note_menu as note_menu, adjustment.jum_stok as jum_stok, menu.* ');
      //$this->db->select('stok_barang.*,menu.*');
      $this->db->from('adjustment');
      $this->db->JOIN('menu','menu.kd_menu=adjustment.kd_menu');
      $this->db->like('adjustment.note_menu',$nama_menu);
      $this->db->like('menu.nama',$nama_menu);
      $query=$this->db->get();
      return $query->result();
    }
    ...
    /*method update data stok*/
    function update_data_adjustment($data=array(),$kd_menu){
      $this->db->where('kd_adjustment',$kd_menu);
      $this->db->update('adjustment', $data);
      $retVal = ($this->db->affected_rows()!=0) ? true : false ;
      return $retVal;
      //return $this->db->update('menu', $data);
    }
    ...
    /*method hapus stok*/
    function delete_data_adjustment($kd_menu){
      $this->db->where('kd_adjustment',$kd_menu);
      $this->db->delete('adjustment');
      $retVal = ($this->db->affected_rows()!=0) ? true : false ;
      return $retVal; 
    } 
