...
class Admin extends CI_Controller {
	...
	*/
	...
	/*update by ajax*/
	public function update_detail_pesanan($no){
		$data=$this->admin_data->getdetail_pesanan($no);
		$pengambilan=($data[0]->metode_pengambilan==0)?'Ambil Sendiri':'Antar';
		$no_pesanan=$no;
		$id_user=$this->input->get("id_user");
        ...
        foreach ($data as $pesanan) {
        	...
        	echo "</td></tr>"
              ...
              ."<tr>"
                	...
                	."<td class=''>";
        foreach ($data as $pesanan) {
        	echo $pesanan->qty.' bks<br>';
          //."<input type='text' class='form-control' id='pesanan' name='pesanan' value=".$pesanan->qty.">"
        }
        	echo "</td></tr>"
	        	."<tr>"
                	."<td>"
	...
	*/
	...
	public function tambah_menu(){
		...
		$harga_menu=$this->input->post('harga_menu', true);
      	...
      	$stok_menu=$this->input->post('stok_menu', true);
		if (empty($harga_menu) or empty($harga_menu) or empty($keterangan)  or empty($status_menu)) {
			header("Location:".base_url('admin/kelola_menu?error=gagal menambah menu'));
		}else{
			$data['nama']=$nama_menu;
			$data['keterangan']=$keterangan;
			$data['harga']=$harga_menu;
          	$data['status']=$status_menu;
          	$data['stok']=$stok_menu;
	...
	public function update_menu_form($kd_menu){
		...
		echo "<form action='".base_url('admin/update_menu')."' enctype='multipart/form-data' method='POST'>"
	        	...
	        	."</div>"
                ."<div class='form-group'>"
                      ."<label>Status Tampilan</label><br>"
          			  ."<input type='radio' id='1' name='status_menu' value='1'  >"
                      ."<label for='female'>Aktif</label><br>"
                      ."<input type='radio' id='0' name='status_menu' value='0' checked>"
                      ."<label for='other'>Tidak Aktif</label>"
                ."</div>"
          		."<div class='form-group'>"
	        		."<label>Stok</label>"
	        		."<input value='".$data[0]->stok."' type='number' name='stok_menu' class='form-control input-sm' required>"
	        	."</div>"
	        	."<div class='form-group'>"
	        		."<label>Picture</label>"
	...
	public function update_menu(){
		...
		if (!empty($kd_menu)) {
			if (!empty($nama_menu))
				$data['nama']=$nama_menu;
			if (!empty($harga_menu)||$harga_menu==0)
				$data['harga']=$harga_menu;
			if (!empty($keterangan)) 
				$data['keterangan']=$keterangan;
            if (!empty($status_menu)||$status_menu==0)
				$data['status']=$status_menu;
          if (!empty($stok_menu)||$stok_menu==0)
				$data['stok']=$stok_menu;
			//upload
			$config['allowed_types']        = 'jpg|jpeg|JPG|JPEG|PNG|png';
	        $config['max_size']             = 2000;
	...
	*/
	...
	/*view Setting jenis,nama, set menu*/
	public function kelola_set_menu(){
		$limit=30;
		$total_rows=$this->admin_data->getcount_menu();
		$paging_url=base_url('admin/kelola_set_menu?');
		$page=$this->paging($this->input->get('per_page'),$limit,$total_rows,$paging_url);
		$data['menu']=$this->admin_data->get_data_setmenu($limit,$page['page']);
		$data['link']=$page['link'];
		$data['jml_psn_msk']=$this->admin_data->getcount_pesanan_masuk();
		$data['jml_knfr_msk']=$this->admin_data->getcount_konfirm();
		$this->load->view('admin_vw/meta_admin');
		...
		$this->load->view('admin_vw/navbar_admin');
		$this->load->view('admin_vw/sidebar_admin',$data);
		$this->load->view('admin_vw/setmenu_admin');
		$this->load->view('admin_vw/tambah_setmenu_admin');
		$this->load->view('admin_vw/jscon_admin');
	}
	/*view Setting jenis,nama, set menu*/
	public function kelola_nama_set(){
		$limit=30;
		$total_rows=$this->admin_data->getcount_menu();
		$paging_url=base_url('admin/kelola_nama_set?');
		$page=$this->paging($this->input->get('per_page'),$limit,$total_rows,$paging_url);
		$data['menu']=$this->admin_data->get_data_namaset($limit,$page['page']);
		$data['link']=$page['link'];
		$data['jml_psn_msk']=$this->admin_data->getcount_pesanan_masuk();
		$data['jml_knfr_msk']=$this->admin_data->getcount_konfirm();
		$this->load->view('admin_vw/meta_admin');
		...
		$this->load->view('admin_vw/navbar_admin');
		$this->load->view('admin_vw/sidebar_admin',$data);
		$this->load->view('admin_vw/namaset_admin');
		$this->load->view('admin_vw/tambah_namaset_admin');
		$this->load->view('admin_vw/jscon_admin');
	}
	/*view Setting jenis set */
	public function kelola_jenis_set(){
		$limit=30;
		$total_rows=$this->admin_data->getcount_menu();
		$paging_url=base_url('admin/kelola_jenis_set?');
		$page=$this->paging($this->input->get('per_page'),$limit,$total_rows,$paging_url);
		$data['menu']=$this->admin_data->get_data_jenisset($limit,$page['page']);
		$data['link']=$page['link'];
		$data['jml_psn_msk']=$this->admin_data->getcount_pesanan_masuk();
		$data['jml_knfr_msk']=$this->admin_data->getcount_konfirm();
		$this->load->view('admin_vw/meta_admin');
		...
		$this->load->view('admin_vw/navbar_admin');
		$this->load->view('admin_vw/sidebar_admin',$data);
		$this->load->view('admin_vw/jenisset_admin');
		$this->load->view('admin_vw/tambah_jenisset_admin');
		$this->load->view('admin_vw/jscon_admin');
	}
	/*view Setting paket menu*/
	public function kelola_paket_menu(){
		$limit=30;
		$total_rows=$this->admin_data->getcount_menu();
		$paging_url=base_url('admin/kelola_paket_menu?');
		$page=$this->paging($this->input->get('per_page'),$limit,$total_rows,$paging_url);
		$data['menu']=$this->admin_data->get_data_paketmenu($limit,$page['page']);
		$data['link']=$page['link'];
		$data['jml_psn_msk']=$this->admin_data->getcount_pesanan_masuk();
		$data['jml_knfr_msk']=$this->admin_data->getcount_konfirm();
		$this->load->view('admin_vw/meta_admin');
		...
		$this->load->view('admin_vw/navbar_admin');
		$this->load->view('admin_vw/sidebar_admin',$data);
		$this->load->view('admin_vw/paketmenu_admin');
		$this->load->view('admin_vw/tambah_paketmenu_admin');
		$this->load->view('admin_vw/jscon_admin');
	}
	/*view stock*/
	public function kelola_stok(){
		$limit=30;
		$total_rows=$this->admin_data->getcount_stok();
		$paging_url=base_url('admin/kelola_stok?');
		$page=$this->paging($this->input->get('per_page'),$limit,$total_rows,$paging_url);
		$data['menu']=$this->admin_data->get_data_stok($limit,$page['page']);
		$data['menu1']=$this->admin_data->get_data_menu1($limit,$page['page']);
		$data['link']=$page['link'];
		$data['jml_psn_msk']=$this->admin_data->getcount_pesanan_masuk();
		$data['jml_knfr_msk']=$this->admin_data->getcount_konfirm();
		$this->load->view('admin_vw/meta_admin');
		...
		$this->load->view('admin_vw/navbar_admin');
		$this->load->view('admin_vw/sidebar_admin',$data);
		$this->load->view('admin_vw/stok_admin');
		$this->load->view('admin_vw/tambah_stok_admin');
		$this->load->view('admin_vw/jscon_admin');
	}
	
	public function search_stok(){
		$nama_menu=$this->input->get('nm_menu');
		$data['menu']=$this->admin_data->get_search_stok($nama_menu);
		$data['jml_psn_msk']=$this->admin_data->getcount_pesanan_masuk();
		$data['jml_knfr_msk']=$this->admin_data->getcount_konfirm();
		$this->load->view('admin_vw/meta_admin');
		$this->load->view('admin_vw/js_admin');
	...
	public function tambah_stok(){
		//$data['menu']=$this->admin_data->get_data_menu($limit,$page['page']);
		$nama_menu =$this->input->post('nama_menu', true);
		$lokasi_awal=$this->input->post('lokasi_awal', true);
		$lokasi_tujuan=$this->input->post('lokasi_tujuan', true);
		$pengirim=$this->input->post('pengirim', true);
		$penerima=$this->input->post('penerima', true);
      	$stok_menu=$this->input->post('stok_menu', true);
		if (empty($nama_menu) or empty($lokasi_awal) or empty($lokasi_tujuan)  or empty($pengirim) or empty($penerima) or empty($stok_menu)) {
			header("Location:".base_url('admin/kelola_stok?error=gagal menambah stok'));
		}else{
			$stok_before=$this->admin_data->jum_stok_before($nama_menu);
			$data['kd_menu']=$nama_menu;
			$data['lokasi_asal']=$lokasi_awal;
			$data['lokasi_tujuan']=$lokasi_tujuan;
			$data['pengirim']=$pengirim;
			$data['penerima']=$penerima;
          	$data['jum_stok']=$stok_menu;
			$data2['kd_menu']=$nama_menu;
			$data2['stok']=$stok_before+$stok_menu;
			$data3['id_user']=$this->session->userdata('id_user');
			$data3['kegiatan'] = "tambah Stok -".$nama_menu."-".$lokasi_awal."-".$lokasi_tujuan."-".$pengirim."-".$penerima."-".$stok_menu;
			$data3['ip'] = $this->input->ip_address();
			$oToday = new DateTime();
			$date3['create_time'] = $oToday->getTimestamp();
			//$data2['jum_stok']=$stok_menu;
			
	 		if ($this->admin_data->tambah_stok($data) && $this->admin_data->update_jum_stok($data2) && $this->admin_data->tambah_history($data3)){
	 			header("Location:".base_url('admin/kelola_stok'));
	 		}else{
	 			header("Location:".base_url('admin/kelola_stok?error=gagal menambah stok'));
	 		}
		}
	}
	public function kelola_adjustment(){
		$limit=30;
		$total_rows=$this->admin_data->getcount_adjustment();
		$paging_url=base_url('admin/kelola_adjustment?');
		$page=$this->paging($this->input->get('per_page'),$limit,$total_rows,$paging_url);
		$data['menu']=$this->admin_data->get_data_adjustment($limit,$page['page']);
		$data['menu1']=$this->admin_data->get_data_menu1($limit,$page['page']);
		$data['link']=$page['link'];
		$data['jml_psn_msk']=$this->admin_data->getcount_pesanan_masuk();
		$data['jml_knfr_msk']=$this->admin_data->getcount_konfirm();
		...
		$this->load->view('admin_vw/navbar_admin');
		$this->load->view('admin_vw/sidebar_admin',$data);
		$this->load->view('admin_vw/adjustment_admin');
		$this->load->view('admin_vw/tambah_adjustment_admin');
		$this->load->view('admin_vw/jscon_admin');
	}
	public function search_adjustment(){
		$nama_menu=$this->input->get('nm_menu');
		$data['menu']=$this->admin_data->get_search_stok($nama_menu);
		$data['jml_psn_msk']=$this->admin_data->getcount_pesanan_masuk();
		$data['jml_knfr_msk']=$this->admin_data->getcount_konfirm();
	...
	public function tambah_adjustment(){
		//$data['menu']=$this->admin_data->get_data_menu($limit,$page['page']);
		$nama_menu =$this->input->post('nama_menu', true);
		$note =$this->input->post('note_menu', true); 
      	$stok_menu=$this->input->post('stok_menu', true);
		if (empty($nama_menu) or empty($note_menu) or empty($stok_menu)) {
			header("Location:".base_url('admin/kelola_adjustment?error=gagal menambah adjustment'));
		}else{
			$stok_before=$this->admin_data->jum_stok_before($nama_menu);
			$data['kd_menu']=$nama_menu;
			$data['note_menu']=$note_menu;
          	$data['jum_stok']=$stok_menu;
			$data2['kd_menu']=$nama_menu;
			$data2['stok']=$stok_before-$stok_menu;
			$data3['id_user']=$this->session->userdata('id_user');
			$data3['kegiatan'] = "Mengurangi Stok -".$nama_menu."-".$note_menu."-".$stok_menu;
			$data3['ip'] = $this->input->ip_address();
			$oToday = new DateTime();
			$date3['create_time'] = $oToday->getTimestamp();
			//$data2['jum_stok']=$stok_menu;
			
	 		if ($this->admin_data->tambah_adjustment($data) && $this->admin_data->update_jum_stok($data2) && $this->admin_data->tambah_history($data3)){
	 			header("Location:".base_url('admin/kelola_adjustment'));
	 		}else{
	 			header("Location:".base_url('admin/kelola_adjustment?error=gagal menambah adjustment'));
	 		}
		}
